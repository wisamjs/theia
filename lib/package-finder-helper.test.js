...
beforeEach(function () {
  combinePackages = helper.__get__('combinePackages');
  isObjProp = helper.__get__('isObjProp');
  isNotObjProp = helper.__get__('isNotObjProp');
  filterPackageName = helper.__get__('filterPackageName');
  makeCheckboxObject = helper.__get__('makeCheckboxObject');
  displayMissing = helper.__get__('displayMissing');
  installDeps = helper.__get__('installDeps');
  confirmInstall = helper.__get__('installDeps');
});
...
describe('filterPackageName', function () {

  var test = 'require(\'gulp-sass\')';
  it('should return package name', function () {
    expect(filterPackageName(test)).to.deep.equal('gulp-sass');
  });
});

describe('combinePackages', function () {
  var arr = [
  ...
  it('should work and return an array', function () {
    expect(combinePackages(arr)).to.have.length(6);
    expect(combinePackages(arr)).to.be.an.instanceof(Array);
    expect(combinePackages(arr)[0]).to.deep.equal('gulp-sass');
  });
});
...
describe('installDeps', function () {
  it('should exist', function () {
    expect(installDeps).to.be.a('function');
  });
});
