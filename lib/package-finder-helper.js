...
function promptUser(pckgs) {
  return promptDeps(pckgs).then(function (depAnswers) {
    ...
    var notSelected = nonIntersect(depAnswers.dependencies);
    //TODO: Account for no remaining packages.
    var remaining = pckgs.filter(notSelected);

    return promptdevDeps(remaining).then(function (devDepAnswers) {
      //BUG: Asynch installations not working for both.
      install('save', depAnswers.dependencies);
      install('save-dev', devDepAnswers.devDependencies);
